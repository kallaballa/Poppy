...
void morph_points(std::vector<cv::Point2f> &srcPts1, std::vector<cv::Point2f> &srcPts2, std::vector<cv::Point2f> &dstPts, float s, int cols, int rows);
...
void extract_features(const Mat& img1, const Mat& img2, Mat &foreground1, Mat &foreground2, std::vector<Mat>& dst1, std::vector<Mat>& dst2, Mat& allContours1, Mat& allContours2, vector<vector<vector<Point>>>& collected1, vector<vector<vector<Point>>>& collected2);
