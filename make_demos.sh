#!/bin/bash

set -e

src/poppy -o yalefaces.mkv -f60 yalefacesout/subject01*.png yalefacesout/subject01.glasses.png
mencoder -vf scale=320:320 -ovc x264 -nosound -fps 30 -ofps 6 yalefaces.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/yalefaces.gif

src/poppy -o faces.mkv -f60 some1.png some2.png some3.png some1.png
mencoder -vf scale=250:320 -ovc x264 -nosound -fps 30 -ofps 6 faces.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/faces.gif

src/poppy -o cars.mkv -f60 kindpng1s.png kindpng2s.png kindpng1s.png
mencoder -vf scale=250:160 -ovc x264 -nosound -fps 30 -ofps 6 cars.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/cars.gif

src/poppy -o browns.mkv -s 1.3 -l1 -a1 -f60 amir1.jpg amir2.jpg amir1.jpg
mencoder -vf scale=360:202 -ovc x264 -nosound -fps 30 -ofps 6 browns.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/browns.gif

src/poppy -s1.1 -o catdog.mkv -f60 cat.png dog.png cat.png
mencoder -vf scale=320:320 -ovc x264 -nosound -fps 30 -ofps 6 catdog.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/catdog.gif

src/poppy -o flowers.mkv -s1.5 -f60 flower1.jpg flower2.jpg flower1.jpg
mencoder -vf scale=320:320 -ovc x264 -nosound -fps 30 -ofps 6 flowers.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/flowers.gif

src/poppy -o squarecircle.mkv -s10 -f60 circle.png square.png circle.png
mencoder -vf scale=320:320 -ovc x264 -nosound -fps 30 -ofps 6 squarecircle.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/squarecircle.gif

src/poppy -o numbers.mkv -a0 -l0 -s1000 -f60 1st.png 2nd.png 3rd.png 2nd.png 1st.png
mencoder -vf scale=320:320 -ovc x264 -nosound -fps 30 -ofps 6 numbers.mkv -o low.mp4
ffmpeg -y -i low.mp4 demo/numbers.gif
